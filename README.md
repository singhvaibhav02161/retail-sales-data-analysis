# Retail Sales Data Analysis & Dashboard

![Python](https://img.shields.io/badge/Python-Pandas%20%7C%20Matplotlib-blue)
![SQL](https://img.shields.io/badge/SQL-MySQL-orange)
![BI](https://img.shields.io/badge/BI-Power%20BI-yellow)
![Excel](https://img.shields.io/badge/Excel-Analysis-green)

## Business Objective
Analyze retail transaction data to understand revenue, profit, regional performance, category/product contribution and monthly sales trends.

## Tools & Skills
**Excel · MySQL · Python · Pandas · Matplotlib · Power BI · DAX · Data Cleaning · EDA · KPI Reporting**

## Project Workflow
`Data Validation → Excel Exploration → SQL Analysis → Python EDA → Power BI Modeling → Dashboard`

## Key Business Questions
- Which regions generate the most revenue and profit?
- Which categories/products contribute most to sales?
- How does revenue change over time?
- What is the average order value and profit margin?
- Which areas deserve deeper commercial investigation?

## Power BI Dashboard
### Executive Overview
Revenue · Profit · Orders · AOV · Profit Margin

### Regional Performance
Revenue, profit, order count and margin by region.

### Product & Category Analysis
Top products, category contribution and profitability.

### Trends
Monthly revenue/profit trends with interactive slicers.

See [`power-bi/DASHBOARD_SPEC.md`](power-bi/DASHBOARD_SPEC.md) and [`power-bi/DAX_measures.txt`](power-bi/DAX_measures.txt).

## Repository Structure
```text
retail-sales-data-analysis/
├── data/
│   ├── retail_sales.csv
│   ├── DATASET_NOTE.md
│   └── DATA_DICTIONARY.md
├── excel/
│   └── retail_sales_analysis.xlsx
├── sql/
│   └── analysis_queries.sql
├── python/
│   └── retail_analysis.py
├── power-bi/
│   ├── DAX_measures.txt
│   └── DASHBOARD_SPEC.md
└── PORTFOLIO_OVERVIEW.md
```

## Dataset Note
The repository contains a compact **synthetic sample dataset** for portfolio review. No real company/client data is used. The analysis workflow is designed to scale to larger transaction datasets.

## How to Run the Python Analysis
```bash
pip install -r requirements.txt
cd python
python retail_analysis.py
```

## Portfolio Disclaimer
This project is a portfolio demonstration of analytical workflow and tooling. It should not be presented as confidential or client-owned work.
